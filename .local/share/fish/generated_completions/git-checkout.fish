...
complete -c git-checkout -l progress -l no-progress --description 'Progress status is reported on the standard error stream by default when it i…'
...
complete -c git-checkout -l guess -l no-guess --description 'If <branch> is not found but there does exist a tracking branch in exactly on…'
...
complete -c git-checkout -l recurse-submodules -l no-recurse-submodules --description 'Using --recurse-submodules will update the content of all active submodules a…'
...
complete -c git-checkout -l pathspec-from-file --description 'Pathspec is passed in <file> instead of commandline args.'
complete -c git-checkout -l pathspec-file-nul --description 'Only meaningful with --pathspec-from-file.'
complete -c git-checkout -l literal-pathspecs --description '.'
